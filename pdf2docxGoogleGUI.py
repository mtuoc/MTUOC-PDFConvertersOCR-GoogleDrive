...
def run_ocr(input_pdf, output_docx, log_callback, finish_callback):
    """Handles the heavy lifting of the OCR process to DOCX in a separate thread."""
    try:
        log_callback("[*] Initializing Google Drive connection...")
        service = get_drive_service(log_callback)
        
        # If output file is not specified, generate it automatically with .docx extension
        if not output_docx:
            output_docx = os.path.splitext(input_pdf)[0] + ".docx"
            
        log_callback(f"\n[>] Processing: {os.path.basename(input_pdf)} -> {os.path.basename(output_docx)}")
        ...
        file_id = file.get('id')
        log_callback(f"[*] Document converted. ID: {file_id}")

        log_callback("[*] Waiting for Google OCR to finish...")
        time.sleep(3)

        log_callback(f"[*] Exporting to DOCX format...")
        ...
        log_callback(f"[OK] Successfully saved: {output_docx}")
...
class PDF2MDApp:
    def __init__(self, root):
        ...
        self.root.geometry("1500x750")  # Kept your geometry settings
